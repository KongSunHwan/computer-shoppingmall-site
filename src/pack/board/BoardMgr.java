...
public class BoardMgr { // Board 관련 process
	...
	private int pList = 10; // 한 페이지에 출력될 수
	private int pageSu = 0; // 전체 페이지 수
	
	
	public BoardMgr() { // DB연결
		try {
	...
	public void saveData(BoardBean bean) { //데이터 저장
	...
	public ArrayList<BoardDto> getDataAll(int page, String stype, String sword){ //데이터 검색하기
	...
	public BoardDto getData(String num) { //데이터 가져오기
		BoardDto dto = null;
		try {
	...
	public boolean checkPass(int num, String new_pass) {
		// 수정 작업을 하기 전에 비밀번호 검사 : db에 있는 비밀번호 == new__pass 여부 확인
		boolean b = false;
		try {
	...
	public void saveEdit(BoardBean bean) { //수정 저장
	...
	public void delete(BoardBean bean) { //삭제하기
